  ...
  try {
    ...
    if (p.get('list') === '1') {
      const rows = await query<{
        ...
        FROM plan_uploads u
        LEFT JOIN plan_jobs j ON j.upload_id = u.id
      ...
      const mesinList = await query<{ nama_mesin: string }>(
        `SELECT DISTINCT nama_mesin FROM plan_uploads ORDER BY nama_mesin`
      );

      return NextResponse.json({
        success: true,
        data: rows.map(r => ({ ...r, total_jobs: Number(r.total_jobs) })),
    ...
    const jobs = await query<{
      ...
      FROM plan_jobs j
      JOIN plan_uploads u ON u.id = j.upload_id
      ${where}
      ORDER BY j.no_urut
    `, vals);
    ...
    if (!jobs.length) {
      return NextResponse.json({
        success: true,
        data: { jobs: [], shifts: [], dates: [], nama_mesin: '', minggu_awal: null, minggu_akhir: null, upload_id: null },
      });
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await query('DELETE FROM plan_uploads WHERE id=$1', [id]);
