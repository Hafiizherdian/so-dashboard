...
import {
  Layers, Search, TrendingDown,
  TrendingUp, Package, ChevronUp, ChevronDown,
  AlertCircle, Trash2, Pencil, Check, Filter, X,
} from 'lucide-react';
...
interface FormData {
  ...
  saldo_awal: string; masuk: string; keluar: string;
  periode: string; keterangan: string;
}
...
function FormModal({
  ...
  theme: Theme; mode: 'create' | 'edit';
  initial?: KertasRow; onClose: () => void; onSaved: () => void;
}) {
  const t = tk[theme];
  const [form, setForm] = useState<FormData>(() =>
    initial ? {
      produk: initial.produk, jenis_kertas: initial.jenis_kertas,
      gramasi: String(initial.gramasi), merk: initial.merk,
      lebar: String(initial.lebar), panjang: String(initial.panjang),
      unit: initial.unit, saldo_awal: String(initial.saldo_awal),
      masuk: String(initial.masuk), keluar: String(initial.keluar),
      periode: initial.periode, keterangan: initial.keterangan || '',
    } : EMPTY_FORM
  );
  ...
  const handleSave = async () => {
    ...
    try {
      const body = {
        ...form,
        gramasi: Number(form.gramasi), lebar: Number(form.lebar),
        panjang: Number(form.panjang), saldo_awal: Number(form.saldo_awal),
        masuk: Number(form.masuk), keluar: Number(form.keluar),
      };
      const url    = mode === 'edit' ? `/api/kertas?id=${initial!.id}` : '/api/kertas';
      const method = mode === 'edit' ? 'PUT' : 'POST';
      const r = await apiJson(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      ...
      <div style={{ background: t.cardbg, border: `1px solid ${t.borderCard}`, borderRadius: 13, overflow: 'hidden', boxShadow: t.shadowCard }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ minWidth: 860, width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={10} style={{ padding: 28, textAlign: 'center', color: t.textMuted, fontFamily: FONT_MONO, fontSize: 11 }}>Memuat data…</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={10} style={{ padding: 28, textAlign: 'center', color: t.textMuted, fontFamily: FONT_MONO, fontSize: 11 }}>Belum ada data alokasi kertas</td></tr>
              ) : filtered.map((row, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '8px 10px', textAlign: 'right', color: t.textMuted, fontFamily: FONT_MONO, fontSize: 10 }}>{row.lebar} × {row.panjang}</td>
                    <td style={{ padding: '8px 10px', color: t.textMuted, fontFamily: FONT_MONO, fontSize: 10 }}>{row.unit}</td>
                    <td style={{ padding: '8px 10px', textAlign: 'right', color: t.textMuted, fontFamily: FONT_MONO, fontSize: 11 }}>{row.saldo_awal.toLocaleString('id-ID')}</td>
