...
function fmtNum(n: number): string {
  ...
  // Mengembalikan angka utuh dengan format ribuan Indonesia (contoh: 1.500.000)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', background: t.cardbg, border: `1px solid ${t.borderCard}`, borderRadius: 12, padding: '10px 14px' }}>
        ...
        {filterOpts.weeks
            .slice() // Salin array agar tidak merubah state asli secara direct
            .sort((a, b) => {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 12 }}>
        {[
          ...
          <div key={card.label} style={{ borderRadius: 13, padding: '12px 14px', background: card.bg, border: `1px solid ${card.border}` }}>
            <div style={{ fontSize: 9, fontFamily: FONT_MONO, textTransform: 'uppercase', letterSpacing: '0.1em', color: card.color, fontWeight: 700, marginBottom: 6 }}>{card.label}</div>
            <div style={{ fontSize: 20, fontWeight: 800, color: t.text, fontFamily: FONT_MONO, letterSpacing: '-0.04em', lineHeight: 1 }}>{card.value}</div>
      ...
      <div style={{ background: t.cardbg, border: `1px solid ${t.borderCard}`, borderRadius: 13, padding: '12px 14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 4, background: t.inputBg, borderRadius: 7, padding: 3 }}>
            {(['proses','mesin','week'] as const).map(m => (
              <button key={m} onClick={() => setChartMode(m)} style={{ fontSize: 9, padding: '3px 10px', borderRadius: 5, border: 'none', fontFamily: FONT_MONO, cursor: 'pointer', background: chartMode === m ? t.cardbg : 'transparent', color: chartMode === m ? t.text : t.textMuted, fontWeight: chartMode === m ? 700 : 400, boxShadow: chartMode === m ? '0 1px 3px rgba(0,0,0,0.15)' : 'none' }}>
                {m === 'proses' ? 'Per Proses' : m === 'mesin' ? 'Per Mesin' : 'Per Minggu'}
              </button>
            ))}
          </div>
        </div>
        <ResponsiveContainer width="100%" height={180}>
          <BarChart data={chartData} margin={{ top: 2, right: 4, left: 0, bottom: 0 }} barGap={2}>
            ...
            <XAxis dataKey="label" tick={{ fontSize: 9, fill: t.textMuted, fontFamily: FONT_MONO }} axisLine={false} tickLine={false}
              tickFormatter={v => v.length > 14 ? v.slice(0,14) + '…' : v}/>
            <YAxis tickFormatter={fmtNum} tick={{ fontSize: 9, fill: t.textMuted, fontFamily: FONT_MONO }} axisLine={false} tickLine={false} width={44}/>
            <Tooltip
            ...
            <table style={{ borderCollapse: 'collapse', fontSize: 12, minWidth: '100%' }}>
              <thead>
                <tr>
                  ...
                  <th style={thS}>No LHKP</th>
                  <th style={{ ...thS, minWidth: 180 }}>Output Produk</th>
                  {/* BARIS DI BAWAH INI SUDAH DIPERBAIKI (Koma digunakan, bukan petik satu) */}
                ...
                {sortedDetail.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ ...tdS, color: t.textSub, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis' }}>{row.mesin}</td>
                      <td style={{ ...tdS }}>
                        {isFinish ? (
                      ...
                      <td style={{ ...tdS, color: t.textMuted, fontSize: 10 }}>{row.no_lhkp}</td>
                      <td style={{ ...tdS, color: t.text, maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis' }}>{row.output_produk}</td>
