...
const PreviewPanel = React.memo(function PreviewPanel({ fileId, fileName, t }: { fileId: string; fileName: string; t: Tokens; }) {
  ...
  useEffect(() => {
    ...
    apiJson(`/api/lhkp/${fileId}/preview`) // Pastikan endpoint ini sesuai backend
      .then(r => {
      ...
      <div style={{ background: t.cardbg, border: `1px solid ${t.borderCard}`, borderRadius: 16, width: '100%', maxWidth: 1000, maxHeight: '85vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: `1px solid ${t.border}`, background: t.tableHead, flexShrink: 0, gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flex: 1 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: t.text }}>Preview Data LHKP</div>
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (r.success) {
        ...
        setUploads(prev => [{
          id: upload_id, file_name: file.name, record_count,
          tgl_awal, tgl_akhir, created_at: new Date().toISOString(),
        }, ...prev]);
        setFile(null);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 16, alignItems: 'stretch', width: '100%' }}>
        ...
        <div style={{ flex: isMobile ? 'none' : 1, width: isMobile ? '100%' : undefined, order: isMobile ? 2 : 1, background: t.cardbg, border: `1px solid ${t.borderCard}`, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: t.text }}>Upload File LHKP</div>
              <div style={{ fontSize: 9, color: t.textMuted, fontFamily: FONT_MONO }}>Data akan ditambahkan ke tabel LHKP</div>
            </div>
          </div>
      ...
      <div style={{ background: t.cardbg, border: `1px solid ${t.borderCard}`, overflow: 'hidden' }}>
        <div style={{ padding: '12px 16px', borderBottom: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 24, height: 24, borderRadius: 7, background: '#818cf815', border: '1px solid #818cf828', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <ClipboardCheck size={12} color="#818cf8" />
          </div>
          <div style={{ fontSize: 12, fontWeight: 700, color: t.text }}>Riwayat Upload LHKP</div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ minWidth: 700, width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>
                {['Nama File', 'Periode', 'Records', 'Status', 'Tanggal', 'Aksi'].map(h => (
            ...
            <tbody>
              {uploads.map((row, i) => (
                <tr key={row.id} style={{ background: i % 2 === 1 ? t.tableAlt : 'transparent' }}>
                  <td style={{ padding: '9px 12px', color: t.text, fontFamily: FONT_MONO, fontSize: 11, maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.file_name}</td>
                  <td style={{ padding: '9px 12px', color: t.textSub, fontFamily: FONT_MONO, fontSize: 11, whiteSpace: 'nowrap' }}>{fmtDate(row.tgl_awal)} – {fmtDate(row.tgl_akhir)}</td>
        ...
        <div onClick={e => e.target === e.currentTarget && setDelTarget(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: t.cardbg, border: `1px solid ${t.borderCard}`, borderRadius: 14, padding: 24, maxWidth: 420, width: '100%' }}>
            <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
              ...
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: t.text, marginBottom: 5 }}>Hapus Upload LHKP</div>
                <div style={{ fontSize: 12, color: t.textSub, lineHeight: 1.6 }}>
                  Yakin hapus <strong>{delTarget.file_name}</strong>?<br />
                  <span style={{ color: t.negText, marginTop: 4, display: 'block' }}>{delTarget.record_count.toLocaleString('id-ID')} records akan terhapus permanen.</span>
                </div>
              </div>
            </div>
