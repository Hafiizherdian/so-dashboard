...
import {
  ...
  AlertCircle, Trash2, ClipboardList,
} from 'lucide-react';
...
//   btnPrimaryStyle, btnDangerStyle, btnGhostStyle,
...
const FORMAT_INFO = [
  ...
  { label: 'Shift 1 / Shift 2 / Shift 3', desc: 'Qty per shift per tanggal' },
];
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 18 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: t.textSub, lineHeight: 1.6 }}>
              Yakin hapus <strong>{target.nama_mesin}</strong> periode{' '}
              {fmtDate(target.minggu_awal)} – {fmtDate(target.minggu_akhir)}?<br />
              <span style={{ color: t.negText }}>
                Semua {target.total_jobs} JOP dan data shift akan terhapus permanen.
              </span>
            </div>
          </div>
        </div>
    ...
    <div style={cardStyle(t)}>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ minWidth: 600, width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr>
              {['Mesin', 'Periode', 'JOP', 'File', 'Diupload', 'Aksi'].map(h => (
          ...
          <tbody>
            {uploads.map((row, i) => (
              ...
              >
                <td style={{ padding: '9px 12px', color: t.text, fontFamily: FONT_MONO, fontSize: 11, fontWeight: 600 }}>
                  {row.nama_mesin}
                </td>
                <td style={{ padding: '9px 12px', color: t.textSub, fontFamily: FONT_MONO, fontSize: 11, whiteSpace: 'nowrap' }}>
                  {fmtDate(row.minggu_awal)} – {fmtDate(row.minggu_akhir)}
                </td>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16,  }}>
      ...
      <div style={cardStyle(t)}>
        <div style={cardHeaderStyle(t)}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: t.text }}>
              Upload File Plan Produksi
            </div>
            <div style={{ fontSize: 9, color: t.textMuted, fontFamily: FONT_MONO }}>
              Data JOP dan shift akan diparse otomatis dari format Excel
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            ...
            >
              {uploading ? (
                ...
                <><Upload size={13} /> Upload Plan Produksi</>
              )}
            </button>
          </div>
        </div>
