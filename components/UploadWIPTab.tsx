...
import {
  Upload, FileSpreadsheet, X, CheckCircle, AlertCircle, Trash2, ClipboardList,
} from 'lucide-react';
...
export default function UploadWipTab({ theme }: Props) {
  ...
  const handleFile = (f: File) => {
    if (!/\.(xlsx|xls)$/i.test(f.name)) {
      setMsg({ type: 'err', text: 'Hanya file .xlsx atau .xls yang didukung' });
      return;
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (r.success) {
        setMsg({
          type: 'ok',
          text: `Berhasil: ${r.data.total_jobs} JOP, ${r.data.total_shifts} entri shift dari ${r.data.nama_mesin} (${r.data.minggu_awal} s/d ${r.data.minggu_akhir})`,
        });
        setFile(null);
      ...
      setMsg({ type: 'err', text: 'Koneksi gagal' });
      ...
      <div style={{ padding: '10px 14px', borderRadius: 10, background: t.infoBg, border: `1px solid ${t.infoBorder}`, fontSize: 10, color: t.infoText, fontFamily: FONT_MONO, lineHeight: 1.8 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 8px' }}>
          {[
            ...
            <span key={h} style={{ padding: '1px 6px', borderRadius: 4, background: t.inputBg, border: `1px solid ${t.infoBorder}`, fontSize: 9 }}>{h}</span>
          ))}
        </div>
      </div>
        ...
        <div style={{ padding: 16 }}>
          {msg && (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '8px 12px', borderRadius: 8, marginBottom: 14, background: msg.type === 'ok' ? t.posBg : t.negBg, border: `1px solid ${msg.type === 'ok' ? t.posBorder : t.negBorder}`, color: msg.type === 'ok' ? t.posText : t.negText, fontSize: 11, fontFamily: FONT_MONO }}>
              ...
              <button onClick={() => setMsg(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', padding: 0, flexShrink: 0 }}><X size={11} /></button>
            </div>
          )}
      ...
      <div style={{ background: t.cardbg, border: `1px solid ${t.borderCard}`, borderRadius: 14, overflow: 'hidden', boxShadow: t.shadowCard }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ minWidth: 640, width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>
                {['Nama File', 'Mesin', 'Periode', 'JOP', 'Tanggal Upload', 'Aksi'].map(h => (
                  <th key={h} style={{ padding: '8px 12px', textAlign: h === 'Aksi' ? 'center' : h === 'JOP' ? 'right' : 'left', fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: t.textMuted, borderBottom: `1px solid ${t.border}`, fontFamily: FONT_MONO, background: t.tableHead }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
